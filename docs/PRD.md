...
| NFR-SEC-006 | Must | Compare the pinned host key before target authentication, do not use TOFU, and disable the pinned Paramiko release's known legacy negotiation algorithms. | T04 | Connector ordering, explicit algorithm-policy, and source guard tests. |
...
| NFR-SEC-008 | Must | Create and require access runtime directories as `0700`, create runtime DB/key artifacts as `0600`, and reject symbolic-link/non-regular security-artifact paths. | T03, T05, T07 | Provisioning/access runtime, key-provider, scenario, and database-permission tests. |
...
| NFR-SEC-011 | Should | Make even encrypted-secret container representations safe for incidental debugging and never intentionally print ciphertext. | T05 | `EncryptedSecret` retains version visibility while redacting nonce/ciphertext; recognizable-marker regression test. |
